import { AuthDatasource, CustomError, LoginDto, RegisterUserDto, UserEntity, type User } from '@/domain';
...
export class AuthDatasourceMongoImpl implements AuthDatasource {
	async register(registerUserDto: RegisterUserDto): Promise<User> {
		...
		try {
			...
			//3. Create entity
			return UserEntity.fromObject(newUser) as User;
		} catch (error) {
			throw error;
		}
	}
	...
	async login(loginDto: LoginDto): Promise<User> {
		...
		try {
			...
			//2. Create entity
			return UserEntity.fromObject(userDb) as User;
		} catch (error) {
			throw error;
		}
	}
